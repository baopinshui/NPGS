...
// ... (BuildDataForObject 和其他辅助函数保持不变) ...
...
CelestialData AstroDataBuilder::BuildDataForStar(const Astro::AStar* star)
{
    ...
    // =======================================================
    {
        ...
        InfoGroup g_evo;
        g_evo.items.push_back({ "形成方式", "普通坍缩" /* TODO: map from EStarFrom */ });
        g_evo.items.push_back({ "预期结局", "缓慢熄灭" /* TODO: map from EStarFrom */ });
        g_evo.items.push_back({ "预期残骸", "氦白矮星" /* TODO: map from EStarFrom */ });
...
CelestialData AstroDataBuilder::BuildDataForPlanet(const Astro::APlanet* planet)
{
    ...
    // 页 1: 物理
    InfoPage page_phys;
    page_phys.name = "PHYSICS";
    InfoGroup g1;
    g1.items.push_back({ "总质量", planet->GetMass().str() + " kg" });
    g1.items.push_back({ "半径", FormatScientific(planet->GetRadius(), "m") });
    page_phys.groups.push_back(g1);
    data.push_back(page_phys);

    // 页 2: 成分
    InfoPage page_comp;
    page_comp.name = "COMPOSITION";
    InfoGroup g2;
    g2.items.push_back({ "大气质量", planet->GetAtmosphereMass().str() + " kg" });
    g2.items.push_back({ "海洋质量", planet->GetOceanMass().str() + " kg" });
    g2.items.push_back({ "核心质量", planet->GetCoreMass().str() + " kg" });
    page_comp.groups.push_back(g2);
    data.push_back(page_comp);

    // ... 按照 JSON 文件继续添加轨道、文明等页面 ...
...
std::string AstroDataBuilder::StarPhaseToString(Astro::AStar::EEvolutionPhase phase)
{
    ...
    case EPhase::kNull: return                      "unknow";
        // ... (添加所有枚举的映射) ...
    default: return "未知";
    }
}
